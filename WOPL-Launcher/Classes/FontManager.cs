...
namespace WOPL_Launcher.Classes {
	class FontManager {
		...
		private int LoadEmbeddedFont(string fontName) {
			Stream manifestResourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("WOPL_Launcher.Fonts." + fontName);
			...
			uint num = 0u;
			AddFontMemResourceEx(intPtr, (uint)array.Length, IntPtr.Zero, ref num);
